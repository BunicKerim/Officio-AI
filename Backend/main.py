...
# ======================================================
# === OFFICIO 1 | AUTO TRANSLATE BACKEND – APPEND START ===
# ======================================================
...
# In-Memory Cache (für Officio 1 ausreichend)
TRANSLATION_CACHE = {}

class TranslateBulkRequest(BaseModel):
    texts: List[str]
    target_language: str  # "en" | "fr"

class TranslateBulkResponse(BaseModel):
    translations: List[str]

def _hash(text: str, lang: str) -> str:
    return hashlib.sha256(f"{text}|{lang}".encode("utf-8")).hexdigest()

def _translate_with_ai(text: str, target_language: str) -> str:
    prompt = f"""
You are translating UI text for a professional business web application.
Keep it short, clear and natural.
Do not explain anything.
Do not add quotes.

Translate the following text to {target_language.upper()}:
...
@router.post("/translate-bulk", response_model=TranslateBulkResponse)
def translate_bulk(payload: TranslateBulkRequest):
    translations = []
    ...
    for text in payload.texts:
        ...
        key = _hash(text, payload.target_language)

        if key in TRANSLATION_CACHE:
            translations.append(TRANSLATION_CACHE[key])
            continue

        translated = _translate_with_ai(text, payload.target_language)
        TRANSLATION_CACHE[key] = translated
        translations.append(translated)

    return TranslateBulkResponse(translations=translations)

# Router registrieren
app.include_router(router)

# ======================================================
# === OFFICIO 1 | AUTO TRANSLATE BACKEND – APPEND END ===
# ======================================================
# ======================================================
# === OFFICIO 1 | CORS FIX FOR TRANSLATE – APPEND START ===
# ======================================================
...
app.add_middleware(
    ...
    allow_origins=[
        "http://127.0.0.1:5500",
        "http://localhost:5500",
        "https://officio-ai.netlify.app",  # falls du das nutzt
        "https://officio-ai.onrender.com"
    ],
    allow_credentials=True,
    allow_methods=["*"],
    allow_headers=["*"],
...
# ======================================================
# === OFFICIO 1 | CORS FIX FOR TRANSLATE – APPEND END ===
# ======================================================
